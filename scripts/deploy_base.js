...
const { MARKETS_BASE } = require('./lib/markets.js');
...
async function main() {
  ...
  //await assetStore.set(usdc.address, { minSize: toUnits('10', 6), chainlinkFeed: ADDRESS_ZERO });
  console.log(`Assets configured.`);

  // Markets
  for (const id in MARKETS_BASE) {
    const _market = MARKETS_BASE[id];
