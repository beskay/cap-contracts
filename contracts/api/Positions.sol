...
contract Positions is Roles {
    ...
    function increasePosition(uint256 orderId, uint256 price) public onlyContract {
        // console.log(1);
        ...
        creditFee(orderId, order.user, order.asset, order.market, order.fee, false);
    ...
    function decreasePosition(uint256 orderId, uint256 price) external onlyContract {
        OrderStore.Order memory order = orderStore.get(orderId);
        //MarketStore.Market memory market = marketStore.get(order.market);
        PositionStore.Position memory position = positionStore.get(order.user, order.asset, order.market);
        ...
        creditFee(orderId, order.user, order.asset, order.market, fee, false);
    ...
    function closePositionWithoutProfit(address _asset, string memory _market) external {
        ...
        creditFee(0, user, _asset, _market, fee, false);
